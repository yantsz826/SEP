...
namespace SEP2.Models
{
    public class PwdUtility
    {
        ...
        public Boolean identifyUser(string vendorID)
        {
            ...
            {
                throw new UserInfoException("Unable to identify vendor ID! ");
            }
        ...
        public Boolean sendUserEmail(string password, string email, string outdate)
        {
            ...
            {
                throw new EmailException("Failed to send emial! ");
            }
        ...
        public string getRegisteredEmail(string vendorID)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    cmd.CommandText = "SELECT EMAILADDR FROM FINANCE_WEB_USERS WHERE VENDORNO = @vendorID";
                    SqlParameter vendorIDParam = new SqlParameter("@vendorID", SqlDbType.VarChar, 30);
                    vendorIDParam.Value = vendorID;
                    cmd.CommandType = CommandType.Text;
                    cmd.Connection = con;
            ...
            {
                throw new UserInfoException("Unable to get user email address! ");
            }
        ...
        public Boolean saveChangeToDB(DateTime outDate, string vendorID, string password)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    cmd.CommandText = "UPDATE FINANCE_WEB_USERS SET OUTDATE = @outDate, PASSWORD = @password WHERE VENDORNO = @vendorID";
            ...
            {
                throw new UserInfoException("Unable to active user account! ");
            }
